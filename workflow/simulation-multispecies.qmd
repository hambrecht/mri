...
- **Separate analyses.** With a detection function per species, the RRMSE ranges from `r round(min(separate$rrmse), 2)` to `r round(max(separate$rrmse), 2)`. For moose and deer the two analyses perform similarly. Pooling helps most for elk, the species with the fewest detections, whose separate model has only its own sightings to estimate the shape of the detection function from: RRMSE `r round(pooled$rrmse[pooled$species == "Elk"], 2)` pooled against `r round(separate$rrmse[separate$species == "Elk"], 2)` separate, and bias `r round(pooled$bias_pct[pooled$species == "Elk"], 1)`% against `r round(separate$bias_pct[separate$species == "Elk"], 1)`%.
